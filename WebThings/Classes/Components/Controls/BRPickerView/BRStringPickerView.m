    ...
    if(_resultBlock) {
        if (self.isSingleColumn) {
            if (!self.selectedItem.length) {
                //如果是单列，没有值得话默认第一位点击确认
                self.selectedItem = [_dataSource safeObjectAtIndex:0];
            }
    ...
    if (!_pickerView) {
        ...
        if (self.isSingleColumn) {
            ...
            [self.selectedItems enumerateObjectsUsingBlock:^(NSString *selectedItem, NSUInteger component, BOOL *stop) {
                [[_dataSource safeObjectAtIndex:component] enumerateObjectsUsingBlock:^(id rowItem, NSUInteger rowIdx, BOOL *stop) {
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component {
    if (self.isSingleColumn) {
        ...
        return ((NSArray *)[_dataSource safeObjectAtIndex:component]).count;
    }
}
...
- (NSString*)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component {
    if (self.isSingleColumn) {
        return [_dataSource safeObjectAtIndex:row];
    } else {
...
{
    return self.isSingleColumn ? 40 : 60.f;
}

- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    if (self.isSingleColumn) {
        self.selectedItem = [_dataSource safeObjectAtIndex:row];
    } else {
