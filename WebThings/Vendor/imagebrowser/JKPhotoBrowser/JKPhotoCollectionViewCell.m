...
- (void)refreshCellWithModel:(JKPhotoModel *)model
    ...
    if (imageUrl && [self isValidURLString:imageUrl]) {
        [self.imageView yy_setImageWithURL:[NSURL URLWithString:imageUrl] placeholder:placeholderImage options:0 completion:^(UIImage * _Nullable image, NSURL * _Nonnull url, YYWebImageFromType from, YYWebImageStage stage, NSError * _Nullable error) {
            
        }];
    } else if (model.smallPicurl && [self isValidURLString:model.smallPicurl]) {
        [self.imageView yy_setImageWithURL:[NSURL URLWithString:model.smallPicurl] placeholder:placeholderImage];
